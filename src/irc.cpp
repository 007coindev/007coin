...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #007coincoinTEST\r");
            Send(hSocket, "WHO #007coincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #007coincoin\n");
            // randomly join #007coincoin00-#007coincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #007coincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #007coincoin%02d\r", channel_number).c_str());
        };
